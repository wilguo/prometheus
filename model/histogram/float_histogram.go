...
func detectReset(currIt, prevIt FloatBucketIterator) bool {
	if !prevIt.Next() {
		return true // If no buckets in previous histogram, nothing can be reset.
	}
	prevBucket := prevIt.At()
	if !currIt.Next() {
		return false // No bucket in current, but at least one in previous histogram.
	}
